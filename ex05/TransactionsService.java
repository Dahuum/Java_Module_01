...
public class TransactionsService {
	...
	public int getUsersCount() { return users.getNumberOfUsers(); }
	public User getUserById(int userId) throws UserNotFoundException { return users.getUserById(userId); }
	
	public Transaction getTransactionById(int userId, UUID id) throws UserNotFoundException, TransactionNotFoundException {
            Transaction[] txs = getTransactionsForUser(userId);
            for (Transaction t : txs) {
            if (t.getId().equals(id)) return t;
            }
            throw new TransactionNotFoundException("Transaction not found for that UUID.");
    }
}